...
const app = new PIXI.Application({
    width: 1000,
    height: mobile ? 1200 : 600,
    antialias: true,
    backgroundColor: 0xeeaaaa,
})

;(() => {
    const dvW = window.innerWidth - 20,
        dvH = window.innerHeight - 20
    let scrW = dvW,
        scrH = dvH
...
const width = mobile ? app.view.width : app.view.width - 200
const height = mobile ? app.view.height - 130 : app.view.height
const rad = height / 10 < width / 10 ? height / 10 : width / 10
const targetCircleSizes = [width / 2, height - rad - 15, rad]
const targetSquareSizes = [width - width / 5 - rad, height - rad * 2 - 15, rad * 2]
const targetTrianglePath = [
    width / 5 + (rad * 2) / Math.sqrt(3),
    height - 15,
    width / 5 - (rad * 2) / Math.sqrt(3),
    height - 15,
    width / 5,
...
function onDragEnd() {
    ...
    if (this.figureType === 'triangle') {
        minX = Math.min(targetTrianglePath[0], targetTrianglePath[2], targetTrianglePath[4])
        maxX = Math.max(targetTrianglePath[0], targetTrianglePath[2], targetTrianglePath[4])
        minY = Math.min(targetTrianglePath[1], targetTrianglePath[3], targetTrianglePath[5])
        maxY = Math.max(targetTrianglePath[1], targetTrianglePath[3], targetTrianglePath[5])
...
function createButtons() {
    ...
    if (mobile) {
        bg.drawRect(0, height, width, 200)
    } else {
    ...
    if (mobile) {
        restart.drawRoundedRect(15, height + 25, 170, 40, 15)
    } else {
    ...
    if (mobile) {
        addMore.drawRoundedRect(15, height + 80, 170, 40, 15)
    } else {
...
function createRandomTriangle() {
    ...
    gr.position.set(getRandom(rad / 2, width - rad * 1.7), getRandom(rad, height - rad * 2.5 - maxY))
    gr.rotation = getRandom(0, 360) / 360
